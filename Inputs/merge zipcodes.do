global INPUT "/Users/camilasury/Desktop/Herramientas computacionales/Python Scrappinng "
...
drop n
compress
save "$INPUT/Output/MD_zipcodes.dta", replace

** Datos crimen que bajamos de socrata
import delimited "$INPUT/Input/crime.csv",clear
keep if year==2015
drop id crime_rate
split name, p(,)
...
drop state_n
compress
save "$INPUT/Output/MD_crime.dta", replace

** Merge
use "$INPUT/Output/MD_crime.dta", clear
ren name1 city
merge m:1 city using "$INPUT/Output/MD_zipcodes.dta"
keep if _m==3
...
levelsof zip_m
*tenemos 23 countys. 
*Falta Garret county, lo rellenamos con el zipcode de Oakland, la cede del condado. 
*zipcode Garret: 21550
save "$INPUT/Output/MD_crime_2015.dta", replace

use "$INPUT/Output/MD_crime_2015.dta", clear
...
save "$INPUT/Output/MD_crime_2015_wide.dta", replace
...
foreach file of local manyfiles {
	...
	*genera precipitaciones promedio por mes
	keep if date_time3=="01"
	*nos quedamos con el dia 1
	ren location zip_m
...
merge m:1 month year zip_m using "$INPUT/Output/MD_crime_2015_wide.dta"
keep if _m==3
drop _m

export delimited using "$INPUT/Output/MD_crime_weather.csv", replace

import delimited "/Users/camilasury/Desktop/Herramientas computacionales/Python Scrappinng /Output/MD_crime_weather.csv", encoding(ISO-8859-1)
